 ...
 *
 * IMPORTANT:
 * Gemini is allowed to UNDERSTAND a command.
 * Gemini is NOT allowed to execute anything.
 *
 * The mobile app/backend validates this contract
 * before any database action is performed.
 */
...
/**
 * Check whether an intent is officially supported by Countr.
 */
export function isValidCountrIntent(intent) {
  return (
    typeof intent === "string" &&
    COUNTR_INTENT_SET.has(intent)
  );
}

/**
 * Safely convert AI output into the Countr contract.
 *
...
export function sanitizeCountrIntent(data) {
  ...
  const intent = isValidCountrIntent(data.intent)
    ? data.intent
    : "unknown";
  ...
  return {
    ...
    new_price:
      typeof data.new_price === "number" &&
      Number.isFinite(data.new_price)
        ? data.new_price
        : null,

    discount_percent:
      typeof data.discount_percent === "number" &&
      Number.isFinite(data.discount_percent)
